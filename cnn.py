...
class CapsuleNet(nn.Module):
    ...
    def __init__(self, input_size, classes, routings):
        ...
        self.fc1 = nn.Linear(7*7*16,784)

        self.fc2 = nn.Linear(784,160)
    ...
    def forward(self, x, y=None):
        ...
        x = self.fc1(x)
        x = self.fc2(x)
        logits = torch.matmul(x,torch.zeros((160,10)).scatter_(0,torch.from_numpy(numpy.arange(0,160).reshape(10,16).T),1.).cuda())
        #length = x.norm(dim=-1)
        if y is None:  # during testing, no label given. create one-hot coding using `length`
            index = logits.max(dim=1)[1]
            y = Variable(torch.zeros(logits.size()).scatter_(1, index.view(-1, 1).cpu().data, 1.).cuda())
        y_ind = torch.argmax(y,dim=1)
        mask = torch.zeros_like(x)
        for i in range(mask.shape[0]):
          mask[i][y_ind[i]*16:(y_ind[i]+1)*16] = 1.

        reconstruction = self.decoder((x * mask))
        return logits, reconstruction.view(-1, *self.input_size)
...
def caps_loss(y_true, y_pred, x, x_recon, lam_recon):
    ...
    L = x_ent(y_pred,y_true.argmax(dim=1))
    L_margin = L.mean()
...
def test(model, test_loader, args):
    ...
    test_loss /= 10000
    return test_loss, correct / 10000

def test_fgsm(model, test_loader, args):
    ...
    for x, y in test_loader:
        ...
        y_true = y.data.max(1)[1]
        correct += y_pred_adv.eq(y_true).cpu().sum()
    test_loss /= 10000
    return test_loss, correct /10000
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', default='./result-cnnr')
    ...
    if not (args.testing or args.fgsm):
        ...
        if args.testing:
          ...
          test_loss, test_acc = test_fgsm(model=model, test_loader=test_loader, args=args)
          print('FGSM : test acc = %.4f, test loss = %.5f' % (test_acc, test_loss))
          show_reconstruction(model, test_loader, 50, args)